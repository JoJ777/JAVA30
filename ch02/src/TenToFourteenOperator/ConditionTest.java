 ...
 *
 * 2.说明：
 *  2.1  条件表达式的结果是boolean类型。
 *  2.2  如果条件表达式的结果是true，则执行表达式1.否则，执行表达式2。
 *  2.3  表达式1 和 表达式2 需要是相同的类型或能兼容的类型。
 *
 *  2.4  开发中，凡是可以使用条件运算的位置，都可以改写为if-else。
 *          反之，能使用if-else结构，不一定能改写为条件运算符。
 *
 *       建议：在二者都能使用的情况下，推荐使用条件运算符。因为执行效率稍高。
 * @Author: iEdHu
 * @Create: 2023/3/23 - 23:19
 * @Version: v1.0
 */
public class ConditionTest {
    public static void main(String[] args) {
        ...
        double result  = (2 > 1)?1:2.0; //1->自动类型提升
